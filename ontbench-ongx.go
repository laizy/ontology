...
func BlockGenAsync(issuer *account.Account, blockChan chan<- *types.Block) {
	// 生成N个账户
	// 构造交易向这些账户转一个ont
	...
	for i := 0; i < len(tsTx); i++ {
		ont := uint64(100)
		balance[i] = ont
		mutable := NewTransferTransaction(utils2.XXXXContractAddress, common.ADDRESS_EMPTY, accounts[i].Address, ont, 0, 100000)
	...
	for h := 0; h < H; h++ {
		...
		for i := 0; i < M; i++ {
			...
			if balance[from] > 0 {
				balance[from] -= 1
				balance[to] += 1
				mutable := NewTransferTransaction(utils2.XXXXContractAddress, accounts[from].Address,
...
func getState(addr common.Address) map[string]uint64 {
	...
	b, _ := ledger.DefLedger.GetStorageItem(utils2.OntContractAddress, addr[:])
	if b != nil {
		ongx = binary.LittleEndian.Uint64(b[1:])
	}
