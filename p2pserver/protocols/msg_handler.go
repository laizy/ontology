...
func (self *MsgHandler) HandleSystemMessage(ctx *Context, msg SystemMessage) {
	switch m := msg.(type) {
	case NetworkStart:
		self.start(ctx.Network())
	case PeerConnected:
		self.blockSync.OnAddNode(m.Info.Id.ToUint64())
		self.reconnect.OnAddPeer(ctx.Sender().Info)
	case PeerDisConnected:
		self.blockSync.OnDelNode(m.Info.Id.ToUint64())
		self.reconnect.OnDelPeer(ctx.Sender().Info)
	case NetworkStop:
		self.stop()
	}
}
