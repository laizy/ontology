...
type PeerPool struct {
	...
	P2pMap  map[uint32]kbucket.KadId //value: p2p random id
...
func NewPeerPool(maxSize int, server *Server) *PeerPool {
	return &PeerPool{
		...
		P2pMap:                 make(map[uint32]kbucket.KadId),
...
func (pool *PeerPool) clean() {
	...
	pool.P2pMap = make(map[uint32]kbucket.KadId)
...
func (pool *PeerPool) addP2pId(peerIdx uint32, p2pId kbucket.KadId) {
	pool.lock.Lock()
	defer pool.lock.Unlock()
...
func (pool *PeerPool) getP2pId(peerIdx uint32) (kbucket.KadId, bool) {
	pool.lock.RLock()
	defer pool.lock.RUnlock()
