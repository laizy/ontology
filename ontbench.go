...
func BlockGen(issuer *account.Account) {
	...
	for i := 0; i < len(tsTx); i++ {
		ont := uint64(100)
		balance[i] = ont
		tsTx[i] = NewTransferTransaction(utils2.OntContractAddress, issuer.Address, accounts[i].Address, ont, 0, 100000)
	...
	for h := 0; h <= 10; h++ {
		tsTx := make([]*types.Transaction, 0, 50000)
		for i := 0; i < 50000; i++ {
			from := rand.Int() % N
			to := rand.Int() % N
			if balance[from] > 0 {
				balance[from] -= 1
				balance[to] += 1
				tsTx = append(tsTx, NewTransferTransaction(utils2.OntContractAddress, accounts[from].Address, accounts[to].Address, 1, 0, 100000))
				if err := signTransaction(accounts[from], tsTx[i]); err != nil {
					fmt.Println("signTransaction error:", err)
					os.Exit(1)
				}
			}
		...
		fmt.Println("execute time:", tend.Unix()-tstart.Unix())
	}
	...
	for i := 0; i < N; i++ {
		...
		if state["ont"] != balance[i] {
			fmt.Println("execution error , balance unmarched. expected:%d, got: %d", balance[i], state["ont"])
		}
	}
