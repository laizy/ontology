...
func BlockGenAsync(issuer *account.Account, blockChan chan<- *types.Block) {
	...
	for i := 0; i < len(tsTx); i++ {
		ont := uint64(100)
		balance[i] = ont
		mutable := NewTransferTransaction(utils2.OngContractAddress, issuer.Address, accounts[i].Address, ont, 0, 100000)
	...
	for h := 0; h < H; h++ {
		...
		for i := 0; i < M; i++ {
			...
			if balance[from] > 0 {
				balance[from] -= 1
				balance[to] += 1
				mutable := NewTransferTransaction(utils2.OngContractAddress, accounts[from].Address,
